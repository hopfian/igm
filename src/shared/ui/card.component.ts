...
export class Card {
	...
	static draw(content: string[], title?: string, width: number = 76): void {
		...
		if (title) {
			...
			console.log(
				borderCol(
					`${Theme.symbols.corner_tl}${horiz} ${title} ${horiz.repeat(Math.max(0, topFill))}${Theme.symbols.corner_tr}`,
				),
			);
		} else {
			console.log(
				borderCol(
					`${Theme.symbols.corner_tl}${horiz.repeat(width - 2)}${Theme.symbols.corner_tr}`,
				),
			);
		}
		...
		for (const line of flatLines) {
			const wrapped = Card.wrapText(line, innerWidth);
		...
		console.log(
			borderCol(
				`${Theme.symbols.corner_bl}${horiz.repeat(width - 2)}${Theme.symbols.corner_br}`,
			),
		);
	}
