...
import { loadCookies, extractCsrfToken } from './auth';
import { loadConfig, IGMConfig } from './config';
...
export class IGClient {
    ...
    constructor(cookieFile: string = 'cookies.txt') {
        this.config = loadConfig();
        this.cookieString = loadCookies(cookieFile);
     ...
     */
    private buildHeaders(method: string, isPost: boolean): Record<string, string> {
    ...
    private async enforceHumanTiming(): Promise<void> {
        ...
        if (rpm > 40) {
            clearActiveSpinner();
            console.error(chalk.yellow(`  ⚠ High request rate (${rpm.toFixed(0)} req/min). Adding cooldown...`));
     ...
     */
    public async apiCall(endpoint: string, method: string = 'GET', data?: any, params?: any): Promise<any> {
        ...
        for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
            try {
                ...
                if (status === 401 || status === 403) {
                    clearActiveSpinner();
                    throw new Error(`Authentication failed (${status}). Re-export your cookies.`);
                }

                // Challenge required — special Instagram defense
                if (error.body?.includes?.('challenge_required') || error.message?.includes?.('challenge_required')) {
                    clearActiveSpinner();
                    throw new Error('Instagram challenge required. Open Instagram in your browser, complete the challenge, then re-export cookies.');
                }
                ...
                if (status === 429) {
                    ...
                    if (attempt < this.retryAttempts) {
                        clearActiveSpinner();
                        console.error(chalk.yellow(`  ⚠ Rate limited. Waiting ${retryAfter}s... (attempt ${attempt}/${this.retryAttempts})`));
