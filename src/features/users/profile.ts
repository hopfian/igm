...
import { ProfileInfo, FriendshipStatus, TimelineItem } from '../../models';
...
export class Profile {
    ...
    public async getProfileByUsername(username: string): Promise<ProfileInfo> {
        const response = await this.client.apiCall(`users/web_profile_info/?username=${encodeURIComponent(username)}`, 'GET');
    ...
    public async getFriendshipStatus(userId: string): Promise<FriendshipStatus> {
        const response = await this.client.apiCall(`friendships/show/${userId}/`, 'GET');
        return {
    ...
    public async restrictUser(userId: string): Promise<any> {
        return await this.client.apiCall('restrict_action/restrict/', 'POST', { target_user_id: userId });
    }

    public async muteUser(userId: string): Promise<any> {
        return await this.client.apiCall(`friendships/mute_posts_or_story_from_follow/`, 'POST', {
