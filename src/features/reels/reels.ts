...
export class Reels {
    ...
    async getGlobalReels(): Promise<TimelineItem[]> {
        ...
        const res = await this.client.apiCall('https://www.instagram.com/graphql/query', 'POST', {
            doc_id: queryId,
        ...
        if (res.data?.xdt_api__v1__clips__home__connection_v2?.edges) {
            for (const edge of res.data.xdt_api__v1__clips__home__connection_v2.edges) {
                if (edge.node?.media) {
