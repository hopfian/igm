...
export class Timeline {
    ...
    public async getComments(mediaId: string): Promise<CommentItem[]> {
        const response = await this.client.apiCall(`media/${mediaId}/comments/`, 'GET');
    ...
    public async addComment(mediaId: string, text: string): Promise<any> {
        return await this.client.apiCall(`web/comments/${mediaId}/add/`, 'POST', { comment_text: text });
    }

    public async likePost(mediaId: string): Promise<any> {
        return await this.client.apiCall(`web/likes/${mediaId}/like/`, 'POST', { d: '0' });
    }

    public async unlikePost(mediaId: string): Promise<any> {
        return await this.client.apiCall(`web/likes/${mediaId}/unlike/`, 'POST', { d: '0' });
    }

    public async savePost(mediaId: string): Promise<any> {
        return await this.client.apiCall(`web/save/${mediaId}/save/`, 'POST', { d: '0' });
    }

    public async unsavePost(mediaId: string): Promise<any> {
        return await this.client.apiCall(`web/save/${mediaId}/unsave/`, 'POST', { d: '0' });
    }
