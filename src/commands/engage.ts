...
export const registerEngageCommands = (yargs: any, client: IGClient) => {
    yargs
        .command(['like <id>', 'l'], 'Like a post', (yargs: any) => {
        ...
        .command(['unlike <id>', 'ul'], 'Unlike a post', (yargs: any) => {
            return yargs.positional('id', { describe: 'Media ID', type: 'string' });
        }, async (argv: any) => {
        ...
        .command(['save <id>', 'sv'], 'Save a post', (yargs: any) => {
            return yargs.positional('id', { describe: 'Media ID', type: 'string' });
        }, async (argv: any) => {
        ...
        .command(['unsave <id>'], 'Unsave a post', (yargs: any) => {
            return yargs.positional('id', { describe: 'Media ID', type: 'string' });
        }, async (argv: any) => {
        ...
        .command(['comment <id> <text>', 'cmt'], 'Comment on a post', (yargs: any) => {
            return yargs.positional('id', { describe: 'Media ID', type: 'string' })
            ...
            s.done('comment added');
        })
        .command(['info <id>'], 'View detailed post info', (yargs: any) => {
            return yargs.positional('id', { describe: 'Media ID', type: 'string' });
        }, async (argv: any) => {
            const { handleDataExport } = await import('../ui/output');
            ...
            if (!item) { s?.fail('post not found'); return; }
            if (isExport) { handleDataExport([item], opts); return; }

            s!.done(`@${item.username}`);
            Display.printTimeline([item]);
        });
};
