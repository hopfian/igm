import { SearchUser, ProfileInfo, FriendshipStatus } from '../../models';
...
export function renderSearchResults(users: SearchUser[]): void {
    ...
    for (const u of users) {
        ...
        const followers = u.follower_count ? chalk.dim(` · ${formatNumber(u.follower_count)} followers`) : '';
        console.log(`  ${Theme.secondary(`@${u.username}`)}${verified}${priv} — ${u.full_name}${followers}`);
    }
    console.log();
}
...
export function renderProfile(profile: ProfileInfo): void {
    ...
    const content = [
        ...
        ...(profile.mutual_followers_count ? [`${profile.mutual_followers_count} mutual follower(s)`] : []),
        '',
...
export function renderFriendship(status: FriendshipStatus, username: string): void {
    ...
    if (status.outgoing_request) console.log(`  ${chalk.yellow('⏳')} Pending follow request`);
    if (status.incoming_request) console.log(`  ${chalk.yellow('⏳')} They requested to follow you`);
    console.log();
}
