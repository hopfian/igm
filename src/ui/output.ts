 ...
 */
export function handleCommentExport(comments: any[], opts: OutputOptions): boolean {
    if (!opts.json && !opts.csv && !opts.pipe && !opts.out) return false;
...
function writeToFile(data: any[], filepath: string, label: string, csvFn: (d: any[]) => string): boolean {
    ...
    console.log('');
    console.log(`  ${chalk.green('✓')} ${chalk.green(`Saved ${data.length} ${label} to ${filepath}`)}`);
    console.log('');
...
function toPostCSV(items: any[]): string {
    ...
    const headers = [
        'id', 'username', 'full_name', 'code', 'caption',
        'like_count', 'comment_count', 'view_count', 'media_type',
        'taken_at', 'location', 'has_liked', 'media_count', 'media_urls', 'url'
    ];

    let csv = headers.join(',') + '\n';
    for (const item of items) {
        const takenAt = item.taken_at ? new Date(item.taken_at * 1000).toISOString() : '';
        const row = [
            item.id,
            item.username,
...
function toUserCSV(users: any[]): string {
    ...
    const headers = [
        ...
        'follower_count', 'following_count', 'media_count', 'biography', 'external_url'
    ];
...
function toCommentCSV(comments: any[]): string {
    ...
    const headers = ['id', 'username', 'text', 'like_count', 'reply_count', 'created_at'];

    let csv = headers.join(',') + '\n';
    for (const c of comments) {
        const createdAt = c.created_at ? new Date(c.created_at * 1000).toISOString() : '';
        const row = [
            c.id,
            c.username,
            escapeCsv(c.text),
...
function printPreview(items: any[]) {
    ...
    console.log(`  ${chalk.dim(`╭─ Preview (first ${count}) ${'─'.repeat(40)}`)}`);
    for (let i = 0; i < count; i++) {
        const item = items[i];
        const label = item.username ? `@${item.username}` : `#${item.id || i}`;
        const snippet = (item.caption || item.text || item.full_name || '').replace(/\n/g, ' ');
        const trunc = snippet.length > 50 ? snippet.substring(0, 47) + '...' : snippet;
        console.log(`  ${chalk.dim('│')} ${chalk.cyan(label)} ${chalk.dim('·')} "${chalk.white(trunc)}"`);
    }
